...
def add_xml_text(taggeddir, text, headerdir, outdir):
    ...
    headerf = text
    headerf = headerf.replace("_clean_tagged.txt", "_header.xml")
    headerf = headerf.replace("_tagged.txt", "_header.xml")
...
tagged_files = re.compile(r"\_tagged.txt$", flags=re.IGNORECASE)
